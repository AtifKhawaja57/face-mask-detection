...
if __name__ == "__main__":
    ...
    while True:
        ...
        if not (wearing_mask or temperature <= 37 or emailed):
            cwd = os.getcwd()
            try:
        ...
        cv2.putText(img, "Mask Detection", (20, 50), cv2.FONT_HERSHEY_COMPLEX, 2, (0, 0, 255), 2)
        cv2.putText(img, f"Face Detected: {len(location) > 0}", (30, 100), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 2)
        if len(location) > 0:
            cv2.putText(img, f"Wearing mask: {wearing_mask}", (30, 130), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 2)
            cv2.putText(img, f"Temperature: {temperature}", (30, 160), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 2)
